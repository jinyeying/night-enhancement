...
class ENHANCENET(object) :
    def __init__(self, args):        
        ...
        self.use_decomp_loss = args.use_decomp_loss
        if args.use_decomp_loss == True:
            self.decomp_weight = args.decomp_weight
    ...
    def build_train_model(self):
        ...
        self.trainA = ImageFolder(os.path.join(self.datasetpath, 'trainA'), train_transform)
        self.trainB = ImageFolder(os.path.join(self.datasetpath, 'trainB'), train_transform)
        self.testA  = ImageFolder(os.path.join(self.datasetpath, 'testA'), test_transform)
        self.testB  = ImageFolder(os.path.join(self.datasetpath, 'testB'), test_transform)

        if self.use_decomp_loss:
            self.trainG = ImageFolder(os.path.join(self.datasetpath, 'results_G'), train_transform) 
            # self.trainL = ImageFolder(os.path.join(self.datasetpath, 'results_L'), train_transform) 
            self.trainJ = ImageFolder(os.path.join(self.datasetpath, 'results_J'), train_transform)             

        self.trainA_loader = DataLoader(self.trainA, batch_size=self.batch_size, shuffle=True)
        self.trainB_loader = DataLoader(self.trainB, batch_size=self.batch_size, shuffle=True)
        if self.use_decomp_loss:
            self.trainG_loader = DataLoader(self.trainG, batch_size=self.batch_size, shuffle=False)
            #self.trainL_loader = DataLoader(self.trainL, batch_size=self.batch_size, shuffle=False)
            self.trainJ_loader = DataLoader(self.trainJ, batch_size=self.batch_size, shuffle=False)

        self.testA_loader = DataLoader(self.testA, batch_size=1, shuffle=False)
        self.testB_loader = DataLoader(self.testB, batch_size=1, shuffle=False)

        self.genA2B = ResnetGenerator(input_nc=3, output_nc=3, ngf=self.ch, n_blocks=self.n_res, img_size=self.img_size).to(self.device)
        self.genB2A = ResnetGenerator(input_nc=3, output_nc=3, ngf=self.ch, n_blocks=self.n_res, img_size=self.img_size).to(self.device)
        self.disGA = Discriminator(input_nc=3, ndf=self.ch, n_layers=7).to(self.device)
    ...
    def train(self):
        ...
        if self.resume:
            ...
            if not len(model_list) == 0:
                ...
                self.load_train(os.path.join(self.result_dir, self.dataset, 'model'), start_iter)
                print(" [*] Load SUCCESS")
        ...
        for step in range(start_iter, self.iteration + 1):
            ...
            if self.use_decomp_loss:
                try:
                    real_G, _ = trainG_iter.next()
                except:
                    trainG_iter = iter(self.trainG_loader)
                    real_G, _ = trainG_iter.next()
                real_G = real_G.to(self.device)
                # try:
                #     real_L, _ = trainL_iter.next()
                # except:
                #     trainL_iter = iter(self.trainL_loader)
                #     real_L, _ = trainL_iter.next()
                # real_L = real_L.to(self.device)
                try:
                    real_J, _ = trainJ_iter.next()
                except:
                    trainJ_iter = iter(self.trainJ_loader)
                    real_J, _ = trainJ_iter.next()
                real_J = real_J.to(self.device)
            ...
            G_loss_A =  self.adv_weight * (G_ad_loss_GA + G_ad_cam_loss_GA + G_ad_loss_LA + G_ad_cam_loss_LA) + self.cycle_weight * G_recon_loss_A + self.identity_weight * G_identity_loss_A + self.atten_weight * G_cam_loss_A
            G_loss_B = self.adv_weight * (G_ad_loss_GB + G_ad_cam_loss_GB + G_ad_loss_LB + G_ad_cam_loss_LB) + self.cycle_weight * G_recon_loss_B + self.identity_weight * G_identity_loss_B + self.atten_weight * G_cam_loss_B

            if self.use_decomp_loss:
                loss_decomp =  self.L1_loss(fake_A2B, real_J)
                G_loss_A = G_loss_A + self.decomp_weight * loss_decomp
            ...
            with torch.no_grad(): ##yeying: added 
