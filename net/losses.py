...
"""
def cc_loss(image):
    image_gray = torch.mean(image, dim=1, keepdims=True)
    loss       = (functional.l1_loss(image[:, 0:1, :, :], image_gray) + \
                  functional.l1_loss(image[:, 1:2, :, :], image_gray) + \
                  functional.l1_loss(image[:, 2:3, :, :], image_gray))*(1/3)
    return loss
...
"""
Function to compute VGG16-ExDarkFintuned based loss
Borrowed from 'networks.py' of NighttimeStereo method
Removed the parallel option from PerceptualLossVgg16ExDark
"""
...
class PerceptualLossVgg16ExDark(nn.Module):
    def __init__(self, vgg=None, 
        ...
        if vgg is None:
            ...
            self.vgg = vgg
        self.vgg     = self.vgg.cuda()#nn.DataParallel(self.vgg, device_ids=gpu_ids).cuda()
